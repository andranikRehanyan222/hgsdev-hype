...
<a href="{{ route('powerlikes.reload') }}" class="btn btn-gradient-primary btn-rounded d-inline">
...
@if(Auth::user()->role == 'admin')
<a href="{{ route('powerlikes.edit', ['id' => $value->id]) }}" class="btn btn-gradient-primary btn-rounded float-right">
<i class="flaticon-edit-6"></i> Edit
</a>
@endif

                    <h3>
                        <a href="{{ route('powerlikes.show', ['id' => $value->id]) }}">
                            {{$value->name}}
                        </a>
                    </h3>
