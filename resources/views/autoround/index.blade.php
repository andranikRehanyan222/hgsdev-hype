...
<a href="{{ route('autorounds.reload') }}" class="btn btn-gradient-primary btn-rounded d-inline"><i class="flaticon-circle-reload-line"></i> Reload from Fuelgram </a>
@endif
@endsection
...
@if(Auth::user()->role == 'admin')
<a href="{{ route('autorounds.edit', ['id' => $value->id]) }}" class="btn btn-gradient-primary btn-rounded float-right"><i class="flaticon-edit-6"></i> Edit</a>
@endif
                    <h3>
                        <a href="{{ route('autorounds.show', ['id' => $value->id]) }}">
                        {{$value->name}}
                    </a>
                    </h3>
